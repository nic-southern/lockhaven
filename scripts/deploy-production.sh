...
SMTP_URL=${SMTP_URL:-}
SESSION_RECORDING_ROOT=/var/lib/guacamole/recordings # pragma: allowlist secret
SESSION_RECORDING_RETENTION_DAYS=${SESSION_RECORDING_RETENTION_DAYS:-30}
ACCESS_REQUEST_TTL_HOURS=${ACCESS_REQUEST_TTL_HOURS:-8}
SSO_OIDC_ISSUER=${SSO_OIDC_ISSUER:-https://auth.newmarketsecurity.com/realms/nms}
SSO_OIDC_DISCOVERY_URL=${SSO_OIDC_DISCOVERY_URL:-https://auth.newmarketsecurity.com/realms/nms/.well-known/openid-configuration}
SSO_OIDC_CLIENT_ID=${SSO_OIDC_CLIENT_ID:-lockhaven}
SSO_OIDC_CLIENT_SECRET=${SSO_OIDC_CLIENT_SECRET:-}
SSO_OIDC_PROVIDER_ID=${SSO_OIDC_PROVIDER_ID:-sso}
SSO_ALLOWED_DOMAINS=${SSO_ALLOWED_DOMAINS:-}
SSO_CLAIMS_MAP=${SSO_CLAIMS_MAP:-}
SSO_DEFAULT_ORGANIZATION_ID=${SSO_DEFAULT_ORGANIZATION_ID:-}
SSO_REQUIRED=${SSO_REQUIRED:-false}
SSO_TRUST_IDP_MFA=${SSO_TRUST_IDP_MFA:-false}
SSO_SAML_PROVIDER_ID=${SSO_SAML_PROVIDER_ID:-sso-saml}
SSO_SAML_ENTRY_POINT=${SSO_SAML_ENTRY_POINT:-}
SSO_SAML_CERT=${SSO_SAML_CERT:-}
SSO_SAML_AUDIENCE=${SSO_SAML_AUDIENCE:-}
SSO_SAML_METADATA_XML=${SSO_SAML_METADATA_XML:-}
TICKETING_INGEST_URL=${TICKETING_INGEST_URL:-}
TICKETING_INGEST_SECRET=${TICKETING_INGEST_SECRET:-}
EOF
